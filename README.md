...
In a real-world scenario, data would be ingested from various sources (e.g., S3, Azure Blob) using Snowflake's COPY INTO command. For this project, the RAW layer is simulated by directly inserting sample customer data into RAW.CUSTOMER_DATA_RAW using VALUES clauses. This table holds raw, untransformed data, including the target CHURN_LABEL.
...
**Categorical Encoding:** Categorical features like GENDER, INTERNET_SERVICE, CONTRACT_TYPE, and PAYMENT_METHOD are one-hot encoded directly in SQL using IFF statements, transforming them into numerical (boolean) columns suitable for machine learning algorithms.
...
To simulate the end-to-end workflow, a table ANALYTICS.CUSTOMER_CHURN_PREDICTIONS is created to store hypothetical churn probabilities and predicted labels from an external ML model. In a production environment, this table would be populated by the actual model's output.
